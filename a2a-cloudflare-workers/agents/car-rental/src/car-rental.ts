...
import { BookingState, CarCriteria } from './types';
...
export class CarRentalAgent extends BaseAgent {
  ...
  private formatBookingConfirmation(booking: any): string {
    ...
    if (booking.features && booking.features.length > 0) {
      ...
      booking.features.forEach((feature: string) => {
  ...
  private async saveBooking(contextId: string, booking: any): Promise<void> {
    try {
      ...
      await this.env.BOOKINGS.put(
        `car_rental_booking:${booking.booking_reference}`,
      ...
      await this.env.BOOKINGS.put(
        `car_rental_booking_by_context:${contextId}`,
        JSON.stringify(bookingRecord),
