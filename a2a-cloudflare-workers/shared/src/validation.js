...
export class InputValidator {
    ...
    static validateA2AMessage(input) {
        const errors = [];
    ...
    static validateMCPRequest(input) {
        const errors = [];
        if (!input || typeof input !== 'object') {
            return { valid: false, errors: ['Input must be an object'] };
        }
    ...
    static validateMCPMethodParams(method, params) {
        const errors = [];
        switch (method) {
            case 'tools/list':
    ...
    static validateA2ARPCMethod(input) {
        const errors = [];
        // JSON-RPC 2.0 validation
        if (input.jsonrpc !== '2.0') {
            errors.push('jsonrpc must be "2.0"');
    ...
    static validateA2AMethodParams(method, params) {
        const errors = [];
    ...
    static validateMessagePart(part, index) {
        const errors = [];
    ...
    static validateProperty(name, value, schema) {
        const errors = [];
        let sanitized = value;
        switch (schema.type) {
            case 'string':
    ...
    static sanitizeSQLString(input) {
...
export function createValidationMiddleware() {
    return async function validationMiddleware(request, next) {
