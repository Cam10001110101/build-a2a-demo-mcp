...
interface A2ATask {
  ...
  history: A2AMessage[];
  stateTransitions?: A2AStateTransition[];
  artifacts?: any[];
  final?: boolean;
  validation_errors?: string[];
}

interface A2AStateTransition {
  fromState?: 'submitted' | 'working' | 'completed' | 'failed' | 'cancelled' | 'input-required' | 'unknown';
  toState: 'submitted' | 'working' | 'completed' | 'failed' | 'cancelled' | 'input-required' | 'unknown';
  timestamp: string;
  reason?: string;
}
...
export abstract class BaseAgent {
  ...
  protected messages: Map<string, A2AMessage> = new Map();
  protected taskHistory: Map<string, string[]> = new Map(); // contextId -> taskId[]
  
  // KV Storage for persistence (optional - can be undefined for agents without KV)
  protected kvStorage?: KVNamespace;

  constructor(agentCard: AgentCard, authConfig?: AuthConfig) {
    ...
    if (authConfig) {
      ...
      this.agentCard = addSecuritySchemeToAgentCard(agentCard, authConfig) as AgentCard;
    }
  }
  ...
  protected async handleSyncResponse(
    query: string, 
    contextId: string, 
    taskId: string
  ): Promise<Response> {
    let finalResponse: A2AAgentResponse | null = null;
  ...
  protected setKVStorage(kv: KVNamespace): void {
    this.kvStorage = kv;
  }

  // Load task from persistent storage
  protected async loadTask(taskId: string): Promise<A2ATask | null> {
    ...
    if (this.kvStorage) {
      try {
        ...
        if (taskData) {
          const task = JSON.parse(taskData) as A2ATask;
  ...
  protected async saveTask(task: A2ATask): Promise<void> {
  ...
  protected async loadMessage(messageId: string): Promise<A2AMessage | null> {
    // Check in-memory cache first
    ...
    if (this.kvStorage) {
      try {
        ...
        if (messageData) {
          const message = JSON.parse(messageData) as A2AMessage;
  ...
  protected async saveMessage(message: A2AMessage): Promise<void> {
    // Update in-memory cache
  ...
  protected async loadTaskHistory(contextId: string): Promise<string[]> {
    // Check in-memory cache first
  ...
  protected async saveTaskHistory(contextId: string, history: string[]): Promise<void> {
    // Update in-memory cache
  ...
  protected async recordStateTransition(
    task: A2ATask, 
    newState: 'submitted' | 'working' | 'completed' | 'failed' | 'cancelled' | 'input-required' | 'unknown',
    reason?: string
  ): Promise<void> {
  ...
  protected async handleA2ARPCMethod(body: any): Promise<Response> {
  ...
  private async handleMessageStream(body: any): Promise<Response> {
  ...
  private async handleMessageSend(params: any): Promise<any> {
  ...
  private async processTaskAsync(taskId: string, query: string, contextId: string, historyLength?: number): Promise<void> {
  ...
  private async handleTasksGet(params: any): Promise<any> {
    if (!params?.taskId) {
  ...
  private async handleTasksList(params: any): Promise<A2ATask[]> {
  ...
  private async handleTasksCancel(params: any): Promise<{ success: boolean; message: string }> {
    if (!params?.taskId) {
      throw new Error('taskId parameter is required');
    }
  ...
  private async handleTasksResubscribe(params: any): Promise<Response> {
    if (!params?.taskId) {
      throw new Error('taskId parameter is required');
    }
  ...
  private async handleTasksSubmitInput(params: any): Promise<any> {
    if (!params?.taskId) {
      throw new Error('taskId parameter is required');
    }
    if (!params?.input) {
  ...
  private async handleMessagesGet(params: any): Promise<A2AMessage | null> {
    if (!params?.messageId) {
  ...
  private async handleMessagesList(params: any): Promise<A2AMessage[]> {
    const contextId = params?.contextId;
